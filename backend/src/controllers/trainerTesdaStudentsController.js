 ...
 *
 * All trainers assigned to the same course through
 * tesda_course_trainers can see the SAME students.
 *
 * Filters:
 * - course_id
 * - schedule_id
...
async function listMyTesdaStudents(req, res) {
  try {
    ...
    // receive the SAME student records.
    // =====================================================
    ...
    const [rows] = await pool.execute(
      `
        ...
        FROM (
          ...
          /*
           * THIS is the shared trainer access.
           *
           * If Trainer A, B, C, D, E are
           * assigned to the same course,
           * all of them can see these students.
           */
          JOIN ${ASSIGN_TABLE} a
            ON a.course_id =
               ts.course_id
