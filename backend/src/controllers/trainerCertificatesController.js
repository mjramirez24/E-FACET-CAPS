...
async function generateTesdaPdf({
  ...
  if (fileExists(watermarkAbs)) {
    ...
    doc.image(watermarkAbs, wmX, wmY, {
      width: wmW,
      height: wmH,
    });
  ...
  doc
    ...
    .fontSize(8)
    .fillColor("#6b7280")
    .text("Certificate Code", codeX, codeY, {
      width: codeBoxW,
      align: "right",
    })
    .font("Courier-Bold")
...
/* =========================================================
   LIST TESDA COMPLETIONS

   SHARED MULTI-TRAINER ACCESS

   Access is based on:
   tesda_course_trainers

   NOT:
   tesda_schedules.trainer_id
   ========================================================= */
    ...
    const [rows] = await pool.execute(
      `
          ...
          LEFT JOIN certificates cert
            ON cert.reservation_id =
               tr.reservation_id
           AND cert.certificate_type =
               'TESDA'
...
/* =========================================================
   GENERATE TESDA CERTIFICATE

   Any trainer assigned to the COURSE
   may generate the certificate.

   Certificate remains one certificate
   per reservation.
   ========================================================= */
  ...
  try {
    ...
    if (!rr.length) {
      return res.status(404).json({
        ...
        message:
          "TESDA reservation not found, not completed, or the course is not assigned to you.",
      });
    }
    ...
    const [ins] = await pool.execute(
      `
          ...
          `,
      [reservationId, certificate_code, issued_at],
    );
...
/* =========================================================
   CERTIFICATE ACCESS HELPER

   Any trainer currently assigned to the
   reservation's COURSE can view/download.
   ========================================================= */

async function getCertPdfInfoForTrainer(certId, trainerId) {
  const certificateId = Number(certId);

  if (!Number.isFinite(certificateId) || certificateId < 1) {
    return null;
  }
  ...
  if (!fs.existsSync(abs)) {
    return {
      ...
      certificate_code: rows[0].certificate_code,
    };
  }
