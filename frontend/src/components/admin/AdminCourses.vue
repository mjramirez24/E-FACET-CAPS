                ...
                >
                  ...
                  <td>
                    ...
                    <div
                      ...
                      v-if="getValidTesdaAssignments(c.id).length"
                    ...
                    >
                      ...
                      <div
                        ...
                        v-for="a in getValidTesdaAssignments(c.id)"
...
export default {
  ...
  setup() {
    ...
    const fetchTesdaAssignments = async () => {
      ...
      // Used as a fallback when the assignment endpoint does not include
      // the trainer name/code. The trainer list is loaded first on refresh.
      const trainerLookup = new Map(
        tesdaTrainers.value.map((t) => [Number(t.trainer_id), t]),
      );

      for (const r of rows) {
        const courseId = Number(r.course_id);
        const trainerId = Number(r.trainer_id);

        if (!courseId || !trainerId) continue;

        const trainer = trainerLookup.get(trainerId);
        const apiName = String(r.trainer_name || "").trim();
        const fallbackName = String(trainer?.fullname || "").trim();
        const placeholderNames = new Set(["trainer", "assign trainer", "—", "-"]);
        const trainerName = placeholderNames.has(apiName.toLowerCase())
          ? fallbackName
          : apiName || fallbackName;
        const trainerCode = String(
          r.trainer_code || trainer?.trainer_code || "",
        ).trim();

        // Ignore orphan/invalid assignment rows instead of displaying
        // a generic placeholder as if it were an assigned trainer.
        if (
          !trainerName ||
          placeholderNames.has(trainerName.toLowerCase())
        ) continue;
        ...
        map[courseId].push({
          ...
          status: r.status || trainer?.status || "",
        });
      }

      tesdaAssignmentsMap.value = map;

      // Automatically check currently active assigned trainers.
      for (const c of tesdaCourses.value) {
        tesdaPendingAssign[c.id] = getValidTesdaAssignments(c.id).map((a) =>
          Number(a.trainer_id),
        );
      }
    };

    const getValidTesdaAssignments = (courseId) => {
      const assigned = tesdaAssignmentsMap.value[courseId] || [];

      return assigned.filter((a) => {
        const name = String(a.trainer_name || "").trim();
        const placeholderNames = new Set(["trainer", "assign trainer", "—", "-"]);
        const hasTrainer =
          Number(a.trainer_id) > 0 &&
          name !== "" &&
          !placeholderNames.has(name.toLowerCase());
        const active =
          !a.status || String(a.status).toLowerCase() === "active";

        return hasTrainer && active;
      });
    };

    const tesdaAssignmentLabel = (courseId) => {
      const assigned = getValidTesdaAssignments(courseId);

      if (!assigned.length) return "Assign Trainer";

      return assigned
        .map((a) =>
          a.trainer_code
            ? `${a.trainer_name} (${a.trainer_code})`
            : a.trainer_name,
        )
        .join(", ");
    };
    ...
    return {
      ...
      tesdaCourses, tesdaLoading, tesdaSortBy, filteredTesdaCourses, tesdaTrainers, tesdaAssignmentsMap, tesdaPendingAssign, getValidTesdaAssignments, tesdaAssignmentLabel, refreshTesdaAssignments, saveTesdaAssignment,
