<template>
  <TrainerLayout active-page="messages">
    ...
    <div>
      ...
      <div class="flex flex-wrap gap-4 mb-6 p-4 bg-gray-50 rounded-lg">
        <div>
          ...
          <select v-model="selectedType" class="w-48 p-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
        ...
        <div>
          ...
          <select v-model="selectedStatus" class="w-40 p-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
        ...
        <div class="flex items-end gap-2">
          <button @click="clearFilters" class="px-3 py-2 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-50 text-sm font-medium transition-colors">Clear</button>
          <button @click="markAllAsRead" class="px-3 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm font-medium transition-colors">Mark All Read</button>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="flex h-[600px]">
          ...
          <div class="w-1/3 border-r border-gray-200 bg-gray-50 flex flex-col">
            <div class="p-4 bg-blue-800 text-white">
              <div class="flex justify-between items-center">
                ...
                <span class="text-xs bg-blue-600 px-2 py-1 rounded-full">{{ filteredConversations.length }} conversations</span>
              </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto">
              ...
              >
                <div class="flex items-start gap-3">
                  <div class="w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-medium shrink-0" :class="getUserBadgeClass(conversation.role)">
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex justify-between items-start">
                      ...
                      <div class="text-xs text-gray-500 shrink-0 ml-2">{{ formatTime(conversation.lastMessageTime) }}</div>
                    </div>
                    <p class="text-sm text-gray-600 mt-1 truncate">{{ conversation.lastMessage || 'No messages yet' }}</p>
                    <div class="flex items-center gap-2 mt-2">
                      <span v-if="conversation.unreadCount > 0" class="text-xs bg-blue-600 text-white px-2 py-0.5 rounded-full shrink-0">
                        {{ conversation.unreadCount }} new
                      </span>
                      <span class="text-xs px-2 py-0.5 rounded-full shrink-0" :class="getStatusBadgeClass(conversation.status)">
                        {{ conversation.status }}
                      </span>
                    </div>
                  </div>
          ...
          <div class="w-2/3 flex flex-col">
            ...
            <!-- Chat Messages -->
            <div v-if="selectedConversation" ref="messagesContainer" class="flex-1 overflow-y-auto p-4 bg-gray-50">
                ...
                >
                  <div :class="[
                    ...
                    message.sender_id === myId
                      ...
                      ? 'bg-indigo-100 border border-indigo-200 text-gray-800 rounded-bl-none'
        ...
        <div class="p-6">
          ...
          <div class="space-y-4">
            <div>
              ...
              <select v-model="newMessageRecipient" class="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500">
                <option value="" disabled>Select recipient</option>
                <optgroup label="Admins">
                  <option v-for="c in allContacts.filter(c => c.role === 'admin')" :key="c.id" :value="c.id">{{ c.name }}</option>
                </optgroup>
                <optgroup label="Trainers">
                  <option v-for="c in allContacts.filter(c => c.role === 'trainer')" :key="c.id" :value="c.id">{{ c.name }}</option>
                </optgroup>
                <optgroup label="Students">
                  <option v-for="c in allContacts.filter(c => c.role === 'user')" :key="c.id" :value="c.id">{{ c.name }}</option>
                </optgroup>
              </select>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Message</label>
              <textarea v-model="newMessageContent" rows="4" class="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500" placeholder="Type your message here..."></textarea>
            </div>
          </div>
          <div class="flex justify-end gap-2 mt-6">
            <button type="button" @click="closeNewMessageModal" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-50 text-sm font-medium">Cancel</button>
            <button
              @click="sendNewMessage"
              :disabled="!newMessageRecipient || !newMessageContent.trim() || sending"
              class="px-4 py-2 bg-blue-700 text-white rounded-md hover:bg-blue-800 text-sm font-medium disabled:bg-gray-400 disabled:cursor-not-allowed"
            >
  ...
  setup() {
    ...
    const filteredConversations = computed(() => {
      ...
      if (searchQuery.value) {
        ...
        result = result.filter(c => c.name.toLowerCase().includes(q) || c.role.toLowerCase().includes(q) || (c.lastMessage || '').toLowerCase().includes(q))
      }
      if (selectedType.value) result = result.filter(c => c.role === selectedType.value)
    ...
    const roleLabel = (role) => ({ user: 'Student', admin: 'Admin', instructor: 'Instructor', trainer: 'Trainer' }[role] || role)
    const getInitials = (name) => (name || '?').split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)
    const formatTime = (ts) => {
      if (!ts) return ''
      const date = new Date(ts), now = new Date()
      const d = Math.floor((now - date) / 86400000)
      if (d === 0) return date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      if (d === 1) return 'Yesterday'
      if (d < 7) return date.toLocaleDateString([], { weekday: 'short' })
      return date.toLocaleDateString([], { month: 'short', day: 'numeric' })
    }
    const getUserBadgeClass = (role) => ({ admin: 'bg-purple-600', trainer: 'bg-indigo-600', user: 'bg-yellow-600' }[role] || 'bg-gray-500')
    const getStatusBadgeClass = (status) => status === 'unread' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600'
    const clearFilters = () => { searchQuery.value = ''; selectedType.value = ''; selectedStatus.value = '' }
    const markAllAsRead = () => { inbox.value.forEach(c => { c.unreadCount = 0; c.status = 'read' }); messageStats.value.unreadMessages = 0 }
    const selectConversation = (conv) => loadThread(conv)
    const startConversation = (contact) => loadThread(contact)
    const startNewMessage = () => { showNewMessageModal.value = true }
    const closeNewMessageModal = () => { showNewMessageModal.value = false; newMessageRecipient.value = ''; newMessageContent.value = '' }
