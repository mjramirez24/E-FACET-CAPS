        ...
        <div class="flex h-[calc(100vh-13rem)] min-h-[520px] max-h-[720px]">
          ...
          <div
            ...
            <div v-if="recentContacts.length > 0" class="px-4 pb-3">
              <div class="flex gap-3 overflow-x-auto pb-1">
                ...
                >
                  <div
                    class="w-12 h-12 rounded-full mx-auto mb-1 flex items-center justify-center text-white text-sm font-semibold shadow-sm ring-2 ring-white group-hover:ring-green-300 transition-all"
                    :class="getUserBadgeClass(contact.role)"
                  >
                    {{ getInitials(contact.name) }}
                  </div>
                  <p class="text-[10px] font-medium text-gray-700 truncate group-hover:text-green-700">{{ contact.name.split(' ')[0] }}</p>
                </div>
              </div>
            </div>
              ...
              >
                <div class="relative shrink-0">
                  ...
                  <span
                    ...
                    class="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-green-500 border-2 border-white"
                  ></span>
                </div>
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="flex items-center justify-between gap-2">
                    ...
                    <span
                      v-if="conversation.unreadCount > 0"
                      class="text-[10px] font-bold bg-green-600 text-white w-5 h-5 rounded-full flex items-center justify-center shrink-0"
                    >
          ...
          <div
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <p class="text-[10px] mt-1 text-right" :class="message.sender_id === myId ? 'text-green-100' : 'text-gray-400'">
            ...
            <div
              ...
              class="px-4 py-2 bg-green-50 border-t border-green-100 flex items-center justify-between shrink-0"
            >
              <span class="text-xs text-green-700 font-medium">✏️ Editing — fix the text below and hit send</span>
              <button @click="cancelEditDraft" class="text-xs text-green-600 hover:text-green-800 font-semibold">Cancel</button>
            </div>
            ...
            <div v-if="selectedConversation" class="border-t border-gray-200 p-3 bg-white shrink-0">
              <div class="flex gap-2 items-center">
                <input
                  ...
                  class="flex-1 px-4 py-2.5 border border-gray-300 rounded-full text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-400 focus:bg-white transition-colors"
                />
                <button
                  @click="sendMessage"
                  :disabled="!newMessage.trim() || sending"
                  class="w-10 h-10 shrink-0 flex items-center justify-center bg-green-600 text-white rounded-full hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
                  title="Send"
                >
...
export default {
  ...
  setup() {
    ...
    const getUserBadgeClass = (role) => {
      ...
      if (role === 'user') return 'bg-green-600'
      return 'bg-gray-500'
    }
...
.search-input-modern:focus { border-color: #16a34a; }
