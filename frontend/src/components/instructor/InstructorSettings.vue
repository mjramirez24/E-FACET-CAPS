<template>
  <InstructorLayout active-page="settings">
    <!-- Header -->
      ...
      try {
        ...
        if (response.data?.status === 'success' && response.data?.profile) {
          this.profile = {
            ...
            specialization: response.data.profile.specialization || 'Defensive Driving',
            license_no: response.data.profile.license_no || 'LIC-0001',
            branch: response.data.profile.branch || 'Main Branch'
          }
...
.avatar-circle { width: 72px; height: 72px; border-radius: 50%; background: #d1fae5; color: #059669; display: flex; align-items: center; justify-content: center; font-size: 1.4rem; font-weight: 700; flex-shrink: 0; }
